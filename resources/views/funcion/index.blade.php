        ...
        <div class="panel-body">
          ...
          <div class="pull-right">
            <div class="btn-group">
              ...
              <a title="Nuevo Registro" href="{{  route('funcion.create')  }}" class="btn btn-success" >Agregar</a>
            </div>
          </div>
    ...
    <div class="modal-dialog" align="left" >
        <div class="modal-content"  align="left" >
            ...
            <input type="text" class="form-control" name="nombre" pattern=".{0}|.{3,20}" title="Requiere minimo de (3 caracteres)" OnkeyPress="return SoloLetras(event)" onkeyup="javascript:this.value=this.value.toUpperCase();" maxlength="20" required="required">
             ...
             <tbody>
              @if($funciones->count())  
              @foreach($funciones as $instrumento)  
              <tr>
             ...
             <tbody>
              ...
              <td>
                @if ($funcion->nivel==1)
                 ...
                 @elseif ($funcion->nivel==2)
                  <span class="label label-success">ADMINISTRADOR</span>
                @else
                  <span class="label label-danger">NO HAY REGISTRO</span>
                @endif
              </td>
                <td><a title="Ver" class="btn btn-xs btn-info" href="{{action('FuncionController@show', $funcion->id)}}" ><span class="icon icon-eye"></span>Ver</a></td>
               <td>
                  <form action="{{action('FuncionController@destroy', $funcion->id)}}" method="post">
                   @csrf
                   @method("DELETE")
                   <button onclick="return confirm('ESTÁS SEGURO QUE DESEAS ELIMINAR LA FUNCION?');" class="btn btn-xs btn-danger" type="submit"><span class="icon icon-trash">Eliminar</span></button>
                 </form>
                 </td> 
               </tr>
               @endforeach 
